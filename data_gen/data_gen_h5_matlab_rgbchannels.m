...
for i = 1: length(filepath)
    ...
    img_r_input = imresize(imresize(img_r_label, 1/scale, 'bicubic'), [h, w],'bicubic');
    img_g_label = img_g(1:h-mod(h,scale), 1:w-mod(w,scale));
    img_g_input = imresize(imresize(img_g_label, 1/scale, 'bicubic'), [h, w],'bicubic');
    img_b_label = img_b(1:h-mod(h,scale), 1:w-mod(w,scale));
    img_b_input = imresize(imresize(img_b_label, 1/scale, 'bicubic'), [h, w],'bicubic');
